...
@section('content')
    <div class="title clearfix">
        ...
        <div class="text-right"><a href="{{ route('blog_categories_new') }}" class="btn btn-warning" style="color: #fff;">Создать</a></div>
    </div>
                ...
                @foreach($categories as $category)
                    @if(!isset($category->parent_id))
                        <tr>
                            ...
                            <td>@if($category->subcategories->count() > 0)<a href="{{ route('show_blog_category_subcategories', $category->id) }}">{{ $category->locales[0]->name }}@else{{ $category->locales[0]->name }}@endif</a></td>
